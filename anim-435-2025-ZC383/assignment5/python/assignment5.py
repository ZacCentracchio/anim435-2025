...
def main_ui():
    ...
    cube_scale = cmds.floatFieldGrp(label="Cube Scale", value1=cube_scale)
    sphere_scale = cmds.floatFieldGrp(label="Sphere Scale", value1=sphere_scale)
    torus_scale = cmds.floatFieldGrp(label="Torus Scale", value1=torus_scale)
    ...
    def create_and_export(*args):
        ...
        if cube_scale <= 0 | sphere_scale <= 0 | torus_scale <= 0:
            logger.error("All scales must be a positive number")
            return none
        

        c_scale = cmds.floatFieldGrp(cube_scale, q=True, value1=True)
        s_scale = cmds.floatFieldGrp(sphere_scale, q=True, value1=True)
        t_scale = cmds.floatFieldGrp(torus_scale, q=True, value1=True)
        ...
        def export_specific_object(object_name, user_name, file_name):
            cmds.select(object_name)
            export_path = os.path.join(os.getcwd(), "exports", user_name, f"{file_name}.fbx")
        ...
        if asset == "cube":
            export_specific_object(cube, user, asset)
        elif asset == "sphere":
            export_specific_object(sphere, user, asset)
        elif asset == "torus":
            export_specific_object(torus, user, asset)
        elif asset == "all":
